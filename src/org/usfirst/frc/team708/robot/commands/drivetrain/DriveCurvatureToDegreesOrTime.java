 ...
 *
 */
public class DriveCurvatureToDegreesOrTime extends Command {
    ...
    public DriveCurvatureToDegreesOrTime(double xSpeed, double zRotation, boolean isQuickTurn, double goalDegrees, double runTime) {
...
//    	constant-curvature turning for turn-in-place maneuvers.
    ...
    protected boolean isFinished() {
    	if (goalDegrees >= 0) {
    		return (Robot.drivetrain.getAngle() >= goalDegrees || this.timeSinceInitialized() >= runTime);
    	} else {
    		return (Robot.drivetrain.getAngle() <= goalDegrees || this.timeSinceInitialized() >= runTime);
    	}
    }
